...
class LandscapeBatteryDrawable(private val context: Context, frameColor: Int) : Drawable() {
    ...
    private var attributionColor: Int = Color.WHITE

    init {
        val res = context.resources
        val density = res.displayMetrics.density

        val widthId = res.getIdentifier("status_bar_battery_icon_landscape_width", "dimen", context.packageName)
        val heightId = res.getIdentifier("status_bar_battery_icon_landscape_height", "dimen", context.packageName)
        val textSizeId = res.getIdentifier("status_bar_battery_icon_landscape_text_size", "dimen", context.packageName)
        val radiusId = res.getIdentifier("status_bar_battery_icon_landscape_radius", "dimen", context.packageName)
    ...
    override fun draw(c: Canvas) {
        ...
        if (attribution != null) {
            BatteryAttributionRenderer.draw(c, attribution, attrRect(attribution), attributionColor)
        }
    ...
    fun getAttributionExtraWidth(heightPx: Int): Int {
        ...
        val glyphW = g.width * (heightPx / BATTERY_INNER_HEIGHT)
        return ceil(ATTR_VISIBLE_FRACTION * glyphW).toInt()
    }

    private fun attributionGlyph(): BatteryAttributionGlyph? = when {
        powerSaveEnabled -> BatteryAttributionGlyph.PLUS
        charging -> BatteryAttributionGlyph.BOLT
        else -> null
    }

    private fun attrRect(g: BatteryAttributionGlyph): RectF {
        val scale = bounds.height() / BATTERY_INNER_HEIGHT
        val glyphW = g.width * scale
        val glyphH = g.height * scale
        val left = fillRect.right - ATTR_OVERLAP * glyphW
        val top = fillRect.centerY() - glyphH / 2f
        return RectF(left, top, left + glyphW, top + glyphH)
    }

    companion object {
        private const val CRITICAL_LEVEL = 15
        private const val BATTERY_INNER_HEIGHT = 13f
